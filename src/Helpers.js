export const options = [
    {
        value: 'Abu Dhabi',
        label: 'Abu Dhabi'
    },
    {
        value: 'Afghanistan',
        label: 'Afghanistan'
    },
    {
        value: 'Albania',
        label: 'Albania'
    },
    {
        value: 'Algeria',
        label: 'Algeria'
    },
    {
        value: 'American Samoa',
        label: 'American Samoa'
    },
    {
        value: 'Andorra',
        label: 'Andorra'
    },
    {
        value: 'Angola',
        label: 'Angola'
    },
    {
        value: 'Anguilla',
        label: 'Anguilla'
    },
    {
        value: 'Antigua & Barbuda',
        label: 'Antigua & Barbuda'
    },
    {
        value: 'Argentina',
        label: 'Argentina'
    },
    {
        value: 'Armenia',
        label: 'Armenia'
    },
    {
        value: 'Aruba',
        label: 'Aruba'
    },
    {
        value: 'Australia',
        label: 'Australia'
    },
    {
        value: 'Austria',
        label: 'Austria'
    },
    {
        value: 'Azerbaijan',
        label: 'Azerbaijan'
    },
    {
        value: 'Azores',
        label: 'Azores'
    },
    {
        value: 'Bahamas',
        label: 'Bahamas'
    },
    {
        value: 'Bahrain',
        label: 'Bahrain'
    },
    {
        value: 'Balearic Islands',
        label: 'Balearic Islands'
    },
    {
        value: 'Bangladesh',
        label: 'Bangladesh'
    },
    {
        value: 'Barbados',
        label: 'Barbados'
    },
    {
        value: 'Belarus',
        label: 'Belarus'
    },
    {
        value: 'Belgium',
        label: 'Belgium'
    },
    {
        value: 'Belize',
        label: 'Belize'
    },
    {
        value: 'Benin',
        label: 'Benin'
    },
    {
        value: 'Bermuda',
        label: 'Bermuda'
    },
    {
        value: 'Bhutan',
        label: 'Bhutan'
    },
    {
        value: 'Bolivia',
        label: 'Bolivia'
    },
    {
        value: 'Bonaire',
        label: 'Bonaire'
    },
    {
        value: 'Bosnia & Herzegovina',
        label: 'Bosnia & Herzegovina'
    },
    {
        value: 'Botswana',
        label: 'Botswana'
    },
    {
        value: 'Brazil',
        label: 'Brazil'
    },
    {
        value: 'British Virgin Islands',
        label: 'British Virgin Islands'
    },
    {
        value: 'Brunei',
        label: 'Brunei'
    },
    {
        value: 'Bulgaria',
        label: 'Bulgaria'
    },
    {
        value: 'Burkina Faso',
        label: 'Burkina Faso'
    },
    {
        value: 'Burma/Myanmar',
        label: 'Burma/Myanmar'
    },
    {
        value: 'Burundi',
        label: 'Burundi'
    },
    {
        value: 'Cambodia',
        label: 'Cambodia'
    },
    {
        value: 'Cameroon',
        label: 'Cameroon'
    },
    {
        value: 'Canada',
        label: 'Canada'
    },
    {
        value: 'Canary Islands',
        label: 'Canary Islands'
    },
    {
        value: 'Cape Verde',
        label: 'Cape Verde'
    },
    {
        value: 'Cayman Islands',
        label: 'Cayman Islands'
    },
    {
        value: 'Central African Republic',
        label: 'Central African Republic'
    },
    {
        value: 'Chad',
        label: 'Chad'
    },
    {
        value: 'Channel Islands',
        label: 'Channel Islands'
    },
    {
        value: 'Chile',
        label: 'Chile'
    },
    {
        value: 'China',
        label: 'China'
    },
    {
        value: 'Christmas Island',
        label: 'Christmas Island'
    },
    {
        value: 'Cocos Islands',
        label: 'Cocos Islands'
    },
    {
        value: 'Colombia',
        label: 'Colombia'
    },
    {
        value: 'Comoros',
        label: 'Comoros'
    },
    {
        value: 'Congo, Democratic Republic',
        label: 'Congo, Democratic Republic'
    },
    {
        value: 'Congo',
        label: 'Congo'
    },
    {
        value: 'Cook Islands',
        label: 'Cook Islands'
    },
    {
        value: 'Costa Rica',
        label: 'Costa Rica'
    },
    {
        value: 'Ivory Coast',
        label: 'Ivory Coast'
    },
    {
        value: 'Croatia',
        label: 'Croatia'
    },
    {
        value: 'Cuba',
        label: 'Cuba'
    },
    {
        value: 'Curaçao',
        label: 'Curaçao'
    },
    {
        value: 'Cyprus',
        label: 'Cyprus'
    },
    {
        value: 'Cyprus, North',
        label: 'Cyprus, North'
    },
    {
        value: 'Czech Republic',
        label: 'Czech Republic'
    },
    {
        value: 'Denmark',
        label: 'Denmark'
    },
    {
        value: 'Djibouti',
        label: 'Djibouti'
    },
    {
        value: 'Dominica',
        label: 'Dominica'
    },
    {
        value: 'Dominican Republic',
        label: 'Dominican Republic'
    },
    {
        value: 'Dubai',
        label: 'Dubai'
    },
    {
        value: 'East Timor',
        label: 'East Timor'
    },
    {
        value: 'Ecuador',
        label: 'Ecuador'
    },
    {
        value: 'Egypt',
        label: 'Egypt'
    },
    {
        value: 'El Salvador',
        label: 'El Salvador'
    },
    {
        value: 'England',
        label: 'England'
    },
    {
        value: 'Equatorial Guinea',
        label: 'Equatorial Guinea'
    },
    {
        value: 'Eritrea',
        label: 'Eritrea'
    },
    {
        value: 'Estonia',
        label: 'Estonia'
    },
    {
        value: 'Ethiopia',
        label: 'Ethiopia'
    },
    {
        value: 'Faeroe Islands',
        label: 'Faeroe Islands'
    },
    {
        value: 'Falkland Islands',
        label: 'Falkland Islands'
    },
    {
        value: 'Fiji',
        label: 'Fiji'
    },
    {
        value: 'Finland',
        label: 'Finland'
    },
    {
        value: 'France',
        label: 'France'
    },
    {
        value: 'French Guiana',
        label: 'French Guiana'
    },
    {
        value: 'Gabon',
        label: 'Gabon'
    },
    {
        value: 'Gambia',
        label: 'Gambia'
    },
    {
        value: 'Gaza Strip',
        label: 'Gaza Strip'
    },
    {
        value: 'Georgia',
        label: 'Georgia'
    },
    {
        value: 'Germany',
        label: 'Germany'
    },
    {
        value: 'Ghana',
        label: 'Ghana'
    },
    {
        value: 'Gibraltar',
        label: 'Gibraltar'
    },
    {
        value: 'Great Britain',
        label: 'Great Britain'
    },
    {
        value: 'Greece',
        label: 'Greece'
    },
    {
        value: 'Greenland',
        label: 'Greenland'
    },
    {
        value: 'Grenada',
        label: 'Grenada'
    },
    {
        value: 'Guadeloupe',
        label: 'Guadeloupe'
    },
    {
        value: 'Guam',
        label: 'Guam'
    },
    {
        value: 'Guatemala',
        label: 'Guatemala'
    },
    {
        value: 'Guinea',
        label: 'Guinea'
    },
    {
        value: 'Guinea-Bissau',
        label: 'Guinea-Bissau'
    },
    {
        value: 'Guyana',
        label: 'Guyana'
    },
    {
        value: 'Haiti',
        label: 'Haiti'
    },
    {
        value: 'Honduras',
        label: 'Honduras'
    },
    {
        value: 'Hong Kong',
        label: 'Hong Kong'
    },
    {
        value: 'Hungary',
        label: 'Hungary'
    },
    {
        value: 'Iceland',
        label: 'Iceland'
    },
    {
        value: 'India',
        label: 'India'
    },
    {
        value: 'Indonesia',
        label: 'Indonesia'
    },
    {
        value: 'Iran',
        label: 'Iran'
    },
    {
        value: 'Iraq',
        label: 'Iraq'
    },
    {
        value: 'Ireland',
        label: 'Ireland'
    },
    {
        value: 'Ireland, Northern',
        label: 'Ireland, Northern'
    },
    {
        value: 'Isle of Man',
        label: 'Isle of Man'
    },
    {
        value: 'Israel',
        label: 'Israel'
    },
    {
        value: 'Italy',
        label: 'Italy'
    },
    {
        value: 'Jamaica',
        label: 'Jamaica'
    },
    {
        value: 'Japan',
        label: 'Japan'
    },
    {
        value: 'Jordan',
        label: 'Jordan'
    },
    {
        value: 'Kazakhstan',
        label: 'Kazakhstan'
    },
    {
        value: 'Kenya',
        label: 'Kenya'
    },
    {
        value: 'Kiribati',
        label: 'Kiribati'
    },
    {
        value: 'Korea, North',
        label: 'Korea, North'
    },
    {
        value: 'Korea, South',
        label: 'Korea, South'
    },
    {
        value: 'Kosovo',
        label: 'Kosovo'
    },
    {
        value: 'Kuwait',
        label: 'Kuwait'
    },
    {
        value: 'Kyrgyzstan',
        label: 'Kyrgyzstan'
    },
    {
        value: 'Laos',
        label: 'Laos'
    },
    {
        value: 'Latvia',
        label: 'Latvia'
    },
    {
        value: 'Lebanon',
        label: 'Lebanon'
    },
    {
        value: 'Lesotho',
        label: 'Lesotho'
    },
    {
        value: 'Liberia',
        label: 'Liberia'
    },
    {
        value: 'Libya',
        label: 'Libya'
    },
    {
        value: 'Liechtenstein',
        label: 'Liechtenstein'
    },
    {
        value: 'Lithuania',
        label: 'Lithuania'
    },
    {
        value: 'Luxembourg',
        label: 'Luxembourg'
    },
    {
        value: 'Macau',
        label: 'Macau'
    },
    {
        value: 'Macedonia',
        label: 'Macedonia'
    },
    {
        value: 'Madagascar',
        label: 'Madagascar'
    },
    {
        value: 'Madeira',
        label: 'Madeira'
    },
    {
        value: 'Malawi',
        label: 'Malawi'
    },
    {
        value: 'Malaysia',
        label: 'Malaysia'
    },
    {
        value: 'Maldives',
        label: 'Maldives'
    },
    {
        value: 'Mali',
        label: 'Mali'
    },
    {
        value: 'Malta',
        label: 'Malta'
    },
    {
        value: 'Marshall Islands',
        label: 'Marshall Islands'
    },
    {
        value: 'Martinique',
        label: 'Martinique'
    },
    {
        value: 'Mauritania',
        label: 'Mauritania'
    },
    {
        value: 'Mauritius',
        label: 'Mauritius'
    },
    {
        value: 'Mayotte',
        label: 'Mayotte'
    },
    {
        value: 'Mexico',
        label: 'Mexico'
    },
    {
        value: 'Micronesia',
        label: 'Micronesia'
    },
    {
        value: 'Moldova',
        label: 'Moldova'
    },
    {
        value: 'Monaco',
        label: 'Monaco'
    },
    {
        value: 'Mongolia',
        label: 'Mongolia'
    },
    {
        value: 'Montenegro',
        label: 'Montenegro'
    },
    {
        value: 'Montserrat',
        label: 'Montserrat'
    },
    {
        value: 'Morocco',
        label: 'Morocco'
    },
    {
        value: 'Mozambique',
        label: 'Mozambique'
    },
    {
        value: 'Namibia',
        label: 'Namibia'
    },
    {
        value: 'Nauru',
        label: 'Nauru'
    },
    {
        value: 'Nepal',
        label: 'Nepal'
    },
    {
        value: 'Netherlands',
        label: 'Netherlands'
    },
    {
        value: 'New Caledonia',
        label: 'New Caledonia'
    },
    {
        value: 'New Zealand',
        label: 'New Zealand'
    },
    {
        value: 'Nicaragua',
        label: 'Nicaragua'
    },
    {
        value: 'Niger',
        label: 'Niger'
    },
    {
        value: 'Nigeria',
        label: 'Nigeria'
    },
    {
        value: 'Niue',
        label: 'Niue'
    },
    {
        value: 'Norfolk Island',
        label: 'Norfolk Island'
    },
    {
        value: 'North Cyprus',
        label: 'North Cyprus'
    },
    {
        value: 'Northern Ireland',
        label: 'Northern Ireland'
    },
    {
        value: 'North Korea',
        label: 'North Korea'
    },
    {
        value: 'Norway',
        label: 'Norway'
    },
    {
        value: 'Oman',
        label: 'Oman'
    },
    {
        value: 'Pakistan',
        label: 'Pakistan'
    },
    {
        value: 'Palau',
        label: 'Palau'
    },
    {
        value: 'Palestine',
        label: 'Palestine'
    },
    {
        value: 'Panama',
        label: 'Panama'
    },
    {
        value: 'Papua New Guinea',
        label: 'Papua New Guinea'
    },
    {
        value: 'Paraguay',
        label: 'Paraguay'
    },
    {
        value: 'Peru',
        label: 'Peru'
    },
    {
        value: 'Philippines',
        label: 'Philippines'
    },
    {
        value: 'Pitcairn Islands',
        label: 'Pitcairn Islands'
    },
    {
        value: 'Poland',
        label: 'Poland'
    },
    {
        value: 'Portugal',
        label: 'Portugal'
    },
    {
        value: 'Puerto Rico',
        label: 'Puerto Rico'
    },
    {
        value: 'Qatar',
        label: 'Qatar'
    },
    {
        value: 'Réunion',
        label: 'Réunion'
    },
    {
        value: 'Romania',
        label: 'Romania'
    },
    {
        value: 'Russia',
        label: 'Russia'
    },
    {
        value: 'Rwanda',
        label: 'Rwanda'
    },
    {
        value: 'Saba',
        label: 'Saba'
    },
    {
        value: 'Saint Barthélemy',
        label: 'Saint Barthélemy'
    },
    {
        value: 'Saint Kitts and Nevis',
        label: 'Saint Kitts and Nevis'
    },
    {
        value: 'Saint Lucia',
        label: 'Saint Lucia'
    },
    {
        value: 'Saint Martin',
        label: 'Saint Martin'
    },
    {
        value: 'Saint Helena',
        label: 'Saint Helena'
    },
    {
        value: 'Sint Eustatius',
        label: 'Sint Eustatius'
    },
    {
        value: 'Sint Maarten',
        label: 'Sint Maarten'
    },
    {
        value: 'Saint Vincent & Grenadines',
        label: 'Saint Vincent & Grenadines'
    },
    {
        value: 'Samoa',
        label: 'Samoa'
    },
    {
        value: 'San Marino',
        label: 'San Marino'
    },
    {
        value: 'São Tomé & Príncipe',
        label: 'São Tomé & Príncipe'
    },
    {
        value: 'Saudi Arabia',
        label: 'Saudi Arabia'
    },
    {
        value: 'Scotland',
        label: 'Scotland'
    },
    {
        value: 'Senegal',
        label: 'Senegal'
    },
    {
        value: 'Serbia',
        label: 'Serbia'
    },
    {
        value: 'Seychelles',
        label: 'Seychelles'
    },
    {
        value: 'Sierra Leone',
        label: 'Sierra Leone'
    },
    {
        value: 'Singapore',
        label: 'Singapore'
    },
    {
        value: 'Slovakia',
        label: 'Slovakia'
    },
    {
        value: 'Slovenia',
        label: 'Slovenia'
    },
    {
        value: 'Solomon Islands',
        label: 'Solomon Islands'
    },
    {
        value: 'Somalia',
        label: 'Somalia'
    },
    {
        value: 'Somaliland',
        label: 'Somaliland'
    },
    {
        value: 'South Africa',
        label: 'South Africa'
    },
    {
        value: 'South Korea',
        label: 'South Korea'
    },
    {
        value: 'South Sudan',
        label: 'South Sudan'
    },
    {
        value: 'Spain',
        label: 'Spain'
    },
    {
        value: 'Sri Lanka',
        label: 'Sri Lanka'
    },
    {
        value: 'Sudan',
        label: 'Sudan'
    },
    {
        value: 'Suriname',
        label: 'Suriname'
    },
    {
        value: 'Swaziland',
        label: 'Swaziland'
    },
    {
        value: 'Sweden',
        label: 'Sweden'
    },
    {
        value: 'Switzerland',
        label: 'Switzerland'
    },
    {
        value: 'Syria',
        label: 'Syria'
    },
    {
        value: 'Tahiti',
        label: 'Tahiti'
    },
    {
        value: 'Taiwan',
        label: 'Taiwan'
    },
    {
        value: 'Tajikistan',
        label: 'Tajikistan'
    },
    {
        value: 'Tanzania',
        label: 'Tanzania'
    },
    {
        value: 'Thailand',
        label: 'Thailand'
    },
    {
        value: 'Togo',
        label: 'Togo'
    },
    {
        value: 'Tokelau',
        label: 'Tokelau'
    },
    {
        value: 'Tonga',
        label: 'Tonga'
    },
    {
        value: 'Trinidad & Tobago',
        label: 'Trinidad & Tobago'
    },
    {
        value: 'Tunisia',
        label: 'Tunisia'
    },
    {
        value: 'Turkey',
        label: 'Turkey'
    },
    {
        value: 'Turkmenistan',
        label: 'Turkmenistan'
    },
    {
        value: 'Turks & Caicos Islands',
        label: 'Turks & Caicos Islands'
    },
    {
        value: 'Tuvalu',
        label: 'Tuvalu'
    },
    {
        value: 'Uganda',
        label: 'Uganda'
    },
    {
        value: 'Ukraine',
        label: 'Ukraine'
    },
    {
        value: 'United Arab Emirates',
        label: 'United Arab Emirates'
    },
    {
        value: 'United Kingdom',
        label: 'United Kingdom'
    },
    {
        value: 'United States of America',
        label: 'United States of America'
    },
    {
        value: 'United States Virgin Islands',
        label: 'United States Virgin Islands'
    },
    {
        value: 'Uruguay',
        label: 'Uruguay'
    },
    {
        value: 'Uzbekistan',
        label: 'Uzbekistan'
    },
    {
        value: 'Vanuatu',
        label: 'Vanuatu'
    },
    {
        value: 'Vatican City',
        label: 'Vatican City'
    },
    {
        value: 'Venezuela',
        label: 'Venezuela'
    },
    {
        value: 'Vietnam',
        label: 'Vietnam'
    },
    {
        value: 'Virgin Islands, British',
        label: 'Virgin Islands, British'
    },
    {
        value: 'Virgin Islands, United States',
        label: 'Virgin Islands, United States'
    },
    {
        value: 'Wales',
        label: 'Wales'
    },
    {
        value: 'Yemen',
        label: 'Yemen'
    },
    {
        value: 'Zambia',
        label: 'Zambia'
    },
    {
        value: 'Zimbabwe',
        label: 'Zimbabwe'
    }
];

export const plugLookup = [
    {
        country: 'Abu Dhabi',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Afghanistan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Albania',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Algeria',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'American Samoa',
        plugTypes: 'A, B, F, I',
        voltage: '120 V'
    },
    {
        country: 'Andorra',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Angola',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Anguilla',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Antigua & Barbuda',
        plugTypes: 'A, B',
        voltage: '230 V'
    },
    {
        country: 'Argentina',
        plugTypes: 'C, I',
        voltage: '220 V'
    },
    {
        country: 'Armenia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Aruba',
        plugTypes: 'A, B, F',
        voltage: '120 V'
    },
    {
        country: 'Australia',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Austria',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Azerbaijan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Azores',
        plugTypes: 'B, C, F',
        voltage: '230 V'
    },
    {
        country: 'Bahamas',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Bahrain',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Balearic Islands',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Bangladesh',
        plugTypes: 'A, C, D, G, K',
        voltage: '220 V'
    },
    {
        country: 'Barbados',
        plugTypes: 'A, B',
        voltage: '115 V'
    },
    {
        country: 'Belarus',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Belgium',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Belize',
        plugTypes: 'A, B, G',
        voltage: '110 V / 220 V'
    },
    {
        country: 'Benin',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Bermuda',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Bhutan',
        plugTypes: 'C, D, G',
        voltage: '230 V'
    },
    {
        country: 'Bolivia',
        plugTypes: 'A, C',
        voltage: '230 V'
    },
    {
        country: 'Bonaire',
        plugTypes: 'A, C',
        voltage: '127 V'
    },
    {
        country: 'Bosnia & Herzegovina',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Botswana',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Brazil',
        plugTypes: 'C, N',
        voltage: '127 V / 220 V'
    },
    {
        country: 'British Virgin Islands',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Brunei',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Bulgaria',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Burkina Faso',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Burma/Myanmar',
        plugTypes: 'A, C, D, G, I',
        voltage: '230 V'
    },
    {
        country: 'Burundi',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Cambodia',
        plugTypes: 'A, C, G',
        voltage: '230 V'
    },
    {
        country: 'Cameroon',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Canada',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Canary Islands',
        plugTypes: 'C, E, F',
        voltage: '230 V'
    },
    {
        country: 'Cape Verde',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Cayman Islands',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Central African Republic',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Chad',
        plugTypes: 'C, D, E, F',
        voltage: '220 V'
    },
    {
        country: 'Channel Islands',
        plugTypes: 'C, G',
        voltage: '230 V'
    },
    {
        country: 'Chile',
        plugTypes: 'C, L',
        voltage: '220 V'
    },
    {
        country: 'China',
        plugTypes: 'A, C, I',
        voltage: '220 V'
    },
    {
        country: 'Christmas Island',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Cocos Islands',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Colombia',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Comoros',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Congo, Democratic Republic',
        plugTypes: 'C, D, E',
        voltage: '220 V'
    },
    {
        country: 'Congo',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Cook Islands',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Costa Rica',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Ivory Coast',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Croatia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Cuba',
        plugTypes: 'A, B, C, L',
        voltage: '110 V / 220 V'
    },
    {
        country: 'Curaçao',
        plugTypes: 'A, B',
        voltage: '127 V'
    },
    {
        country: 'Cyprus',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Cyprus, North',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Czech Republic',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Denmark',
        plugTypes: 'C, E, F, K',
        voltage: '230 V'
    },
    {
        country: 'Djibouti',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Dominica',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Dominican Republic',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Dubai',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'East Timor',
        plugTypes: 'C, E, F, I',
        voltage: '220 V'
    },
    {
        country: 'Ecuador',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Egypt',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'El Salvador',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'England',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Equatorial Guinea',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Eritrea',
        plugTypes: 'C, L',
        voltage: '230 V'
    },
    {
        country: 'Estonia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Ethiopia',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Faeroe Islands',
        plugTypes: 'C, E, F, K',
        voltage: '230 V'
    },
    {
        country: 'Falkland Islands',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Fiji',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Finland',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'France',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'French Guiana',
        plugTypes: 'C, D, E',
        voltage: '220 V'
    },
    {
        country: 'Gabon',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Gambia',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Gaza Strip',
        plugTypes: 'C, H',
        voltage: '230 V'
    },
    {
        country: 'Georgia',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Germany',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Ghana',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Gibraltar',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Great Britain',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Greece',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Greenland',
        plugTypes: 'C, E, F, K',
        voltage: '230 V'
    },
    {
        country: 'Grenada',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Guadeloupe',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Guam',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Guatemala',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Guinea',
        plugTypes: 'C, F, K',
        voltage: '220 V'
    },
    {
        country: 'Guinea-Bissau',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Guyana',
        plugTypes: 'A, B, D, G',
        voltage: '120 V / 240 V'
    },
    {
        country: 'Haiti',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Honduras',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Hong Kong',
        plugTypes: 'G',
        voltage: '220 V'
    },
    {
        country: 'Hungary',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Iceland',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'India',
        plugTypes: 'C, D, M',
        voltage: '230 V'
    },
    {
        country: 'Indonesia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Iran',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Iraq',
        plugTypes: 'C, D, G',
        voltage: '230 V'
    },
    {
        country: 'Ireland',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Ireland, Northern',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Isle of Man',
        plugTypes: 'C, G',
        voltage: '230 V'
    },
    {
        country: 'Israel',
        plugTypes: 'C, H',
        voltage: '230 V'
    },
    {
        country: 'Italy',
        plugTypes: 'C, F, L',
        voltage: '230 V'
    },
    {
        country: 'Jamaica',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Japan',
        plugTypes: 'A, B',
        voltage: '100 V'
    },
    {
        country: 'Jordan',
        plugTypes: 'C, D, F, G, J',
        voltage: '230 V'
    },
    {
        country: 'Kazakhstan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Kenya',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Kiribati',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Korea, North',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Korea, South',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Kosovo',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Kuwait',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Kyrgyzstan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Laos',
        plugTypes: 'A, B, C, E, F',
        voltage: '230 V'
    },
    {
        country: 'Latvia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Lebanon',
        plugTypes: 'C, D, G',
        voltage: '230 V'
    },
    {
        country: 'Lesotho',
        plugTypes: 'M',
        voltage: '220 V'
    },
    {
        country: 'Liberia',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Libya',
        plugTypes: 'C, L',
        voltage: '230 V'
    },
    {
        country: 'Liechtenstein',
        plugTypes: 'C, J',
        voltage: '230 V'
    },
    {
        country: 'Lithuania',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Luxembourg',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Macau',
        plugTypes: 'G',
        voltage: '220 V'
    },
    {
        country: 'Macedonia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Madagascar',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Madeira',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Malawi',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Malaysia',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Maldives',
        plugTypes: 'C, D, G, J, K, L',
        voltage: '230 V'
    },
    {
        country: 'Mali',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Malta',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Marshall Islands',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Martinique',
        plugTypes: 'C, D, E',
        voltage: '220 V'
    },
    {
        country: 'Mauritania',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Mauritius',
        plugTypes: 'C, G',
        voltage: '230 V'
    },
    {
        country: 'Mayotte',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Mexico',
        plugTypes: 'A, B',
        voltage: '127 V'
    },
    {
        country: 'Micronesia',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Moldova',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Monaco',
        plugTypes: 'C, E, F',
        voltage: '230 V'
    },
    {
        country: 'Mongolia',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Montenegro',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Montserrat',
        plugTypes: 'A, B',
        voltage: '230 V'
    },
    {
        country: 'Morocco',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Mozambique',
        plugTypes: 'C, F, M',
        voltage: '220 V'
    },
    {
        country: 'Namibia',
        plugTypes: 'D, M',
        voltage: '220 V'
    },
    {
        country: 'Nauru',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Nepal',
        plugTypes: 'C, D, M',
        voltage: '230 V'
    },
    {
        country: 'Netherlands',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'New Caledonia',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'New Zealand',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Nicaragua',
        plugTypes: 'A',
        voltage: '120 V'
    },
    {
        country: 'Niger',
        plugTypes: 'C, D, E, F',
        voltage: '220 V'
    },
    {
        country: 'Nigeria',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Niue',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Norfolk Island',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'North Cyprus',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Northern Ireland',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'North Korea',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Norway',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Oman',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Pakistan',
        plugTypes: 'C, D',
        voltage: '230 V'
    },
    {
        country: 'Palau',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Palestine',
        plugTypes: 'C, H',
        voltage: '230 V'
    },
    {
        country: 'Panama',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Papua New Guinea',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Paraguay',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Peru',
        plugTypes: 'A, C',
        voltage: '220 V'
    },
    {
        country: 'Philippines',
        plugTypes: 'A, B, C',
        voltage: '220 V'
    },
    {
        country: 'Pitcairn Islands',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Poland',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Portugal',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Puerto Rico',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Qatar',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Réunion',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Romania',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Russia',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Rwanda',
        plugTypes: 'C, J',
        voltage: '230 V'
    },
    {
        country: 'Saba',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Saint Barthélemy',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Saint Kitts and Nevis',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Saint Lucia',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Saint Martin',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Saint Helena',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Sint Eustatius',
        plugTypes: 'A, B, C, F',
        voltage: '110 V / 220 V'
    },
    {
        country: 'Sint Maarten',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Saint Vincent & Grenadines',
        plugTypes: 'A, B, G',
        voltage: '110 V / 230 V'
    },
    {
        country: 'Samoa',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'San Marino',
        plugTypes: 'C, F, L',
        voltage: '230 V'
    },
    {
        country: 'São Tomé & Príncipe',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Saudi Arabia',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Scotland',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Senegal',
        plugTypes: 'C, D, E, K',
        voltage: '230 V'
    },
    {
        country: 'Serbia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Seychelles',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Sierra Leone',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Singapore',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Slovakia',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Slovenia',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Solomon Islands',
        plugTypes: 'G, I',
        voltage: '230 V'
    },
    {
        country: 'Somalia',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Somaliland',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'South Africa',
        plugTypes: 'C, D, M, N',
        voltage: '230 V'
    },
    {
        country: 'South Korea',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'South Sudan',
        plugTypes: 'C, D',
        voltage: '230 V'
    },
    {
        country: 'Spain',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Sri Lanka',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Sudan',
        plugTypes: 'C, D',
        voltage: '230 V'
    },
    {
        country: 'Suriname',
        plugTypes: 'A, B, C, F',
        voltage: '127 V / 230 V'
    },
    {
        country: 'Swaziland',
        plugTypes: 'M',
        voltage: '230 V'
    },
    {
        country: 'Sweden',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Switzerland',
        plugTypes: 'C, J',
        voltage: '230 V'
    },
    {
        country: 'Syria',
        plugTypes: 'C, E, L',
        voltage: '220 V'
    },
    {
        country: 'Tahiti',
        plugTypes: 'C, E',
        voltage: '220 V'
    },
    {
        country: 'Taiwan',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Tajikistan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Tanzania',
        plugTypes: 'D, G',
        voltage: '230 V'
    },
    {
        country: 'Thailand',
        plugTypes: 'A, B, C, O',
        voltage: '230 V'
    },
    {
        country: 'Togo',
        plugTypes: 'C',
        voltage: '220 V'
    },
    {
        country: 'Tokelau',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Tonga',
        plugTypes: 'I',
        voltage: '240 V'
    },
    {
        country: 'Trinidad & Tobago',
        plugTypes: 'A, B',
        voltage: '115 V'
    },
    {
        country: 'Tunisia',
        plugTypes: 'C, E',
        voltage: '230 V'
    },
    {
        country: 'Turkey',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'Turkmenistan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Turks & Caicos Islands',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Tuvalu',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Uganda',
        plugTypes: 'G',
        voltage: '240 V'
    },
    {
        country: 'Ukraine',
        plugTypes: 'C, F',
        voltage: '230 V'
    },
    {
        country: 'United Arab Emirates',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'United Kingdom',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'United States of America',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'United States Virgin Islands',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Uruguay',
        plugTypes: 'C, F, L',
        voltage: '220 V'
    },
    {
        country: 'Uzbekistan',
        plugTypes: 'C, F',
        voltage: '220 V'
    },
    {
        country: 'Vanuatu',
        plugTypes: 'I',
        voltage: '230 V'
    },
    {
        country: 'Vatican City',
        plugTypes: 'C, F, L',
        voltage: '230 V'
    },
    {
        country: 'Venezuela',
        plugTypes: 'A, B',
        voltage: '120 V'
    },
    {
        country: 'Vietnam',
        plugTypes: 'A, C, D',
        voltage: '220 V'
    },
    {
        country: 'Virgin Islands, British',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Virgin Islands, United States',
        plugTypes: 'A, B',
        voltage: '110 V'
    },
    {
        country: 'Wales',
        plugTypes: 'G',
        voltage: '230 V'
    },
    {
        country: 'Yemen',
        plugTypes: 'A, D, G',
        voltage: '230 V'
    },
    {
        country: 'Zambia',
        plugTypes: 'C, D, G',
        voltage: '230 V'
    },
    {
        country: 'Zimbabwe',
        plugTypes: 'D, G',
        voltage: '240 V'
    }
]

export function getPlugType(country) {
    let countryData = plugLookup.find(p => p.country === country.value);
    if (countryData === null) {
        return "";
    }
    return countryData.plugTypes.replace(' ', '').split(',');
}

export function getPlugTypeClass(plugLetter) {
    return "type-" + plugLetter.toLocaleLowerCase().replace(' ', '');
}

export function getCountryCompatibleSockets(countryOne, countryTwo) {
    let countryOnePlugTypes = plugLookup.find(p => p.country === countryOne.value).plugTypes;
    let countryTwoSockets = plugLookup.find(p => p.country === countryTwo.value).plugTypes;
    
    let compatiblePlugs = [];
    for (let plugType of countryOnePlugTypes) {
        let plugData = { plug: plugType, compatibleSockets: [] };
        let compatibleSockets = getCompatibleSocketsForPlug(plugType);
        for(let socket of compatibleSockets) {
            if(countryTwoSockets.indexOf(socket) !== -1) {
                plugData.compatibleSockets.push(socket);
            }
        }

        if(plugData.compatibleSockets.length > 0) {
            compatiblePlugs.push(plugData);
        }
    }
}

function getCompatibleSocketsForPlug(plugType) {
    let compatiblePlugs = [];
    switch (plugType) {
        case "A":
            compatiblePlugs = ["B"];
            break;
        case "C":
            compatiblePlugs = ["E", "F", "H", "J", "K", "L", "N", "O"];
            break;
        case "E":
            compatiblePlugs = ["C", "F", "K", "O"];
            break;
        case "F":
            compatiblePlugs = ["C", "E", "K", "O"];
            break;
        default:
            compatiblePlugs = []
    }

    compatiblePlugs.push(plugType);
    return compatiblePlugs;
}